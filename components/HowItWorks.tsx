  ...
  return (
    <div className="space-y-3 p-4">
      <div className="flex items-center justify-between">
        <span className="text-[0.625rem] font-semibold uppercase tracking-wider text-text-muted">Zonas de auditoría</span>
        <span className="rounded-full bg-brand-soft px-2 py-0.5 text-[0.5625rem] font-medium text-brand">5 zonas</span>
      </div>
      {zones.map((z) => (
        <div key={z.name} className="flex items-center gap-2 rounded-lg bg-bg-muted p-2.5">
          <div className="h-2 w-2 rounded-full bg-brand" />
          <span className="flex-1 text-[0.6875rem] font-medium text-text">{z.name}</span>
          <span className="text-[0.625rem] text-text-muted">{z.weight}%</span>
          <div className="h-1 w-12 rounded-full bg-border">
            <div className="h-full rounded-full bg-brand" style={{ width: `${z.weight * 4}%` }} />
          </div>
        </div>
      ))}
    ...
    <div className="space-y-2 p-3">
      <div className="mb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-[0.5rem] text-text-muted">Tienda Doña Carmen · Zona 10</p>
        </div>
        <span className="text-[0.5625rem] font-medium text-brand">3/5</span>
      </div>
      {items.map((item) => (
        <div key={item.label} className="flex items-center gap-2 rounded-lg bg-bg-muted p-2">
          <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded ${item.checked ? "bg-brand" : "border border-border"}`}>
            {item.checked && (
          ...
          <span className={`text-[0.5625rem] ${item.checked ? "text-text" : "text-text-muted"}`}>{item.label}</span>
        </div>
      ))}
      <div className="mt-2 rounded-lg border border-dashed border-border p-3 text-center">
        <p className="text-[0.5rem] text-text-muted">📷 Toca para agregar foto</p>
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-col items-center p-3">
      ...
      <div className="relative mb-3 flex h-16 w-16 items-center justify-center">
        <svg viewBox="0 0 100 100" className="absolute inset-0 -rotate-90">
          <circle cx="50" cy="50" r="40" fill="none" stroke="#E8E8E8" strokeWidth="6" />
          <circle cx="50" cy="50" r="40" fill="none" stroke="#22c55e" strokeWidth="6" strokeLinecap="round"
            strokeDasharray={2 * Math.PI * 40} strokeDashoffset={2 * Math.PI * 40 * 0.1} />
        </svg>
        <span className="font-display text-[1.125rem] font-bold text-text">90</span>
      </div>
      <div className="w-full space-y-1.5">
        {zones.map((z) => (
          <div key={z.name} className="flex items-center gap-1.5">
            ...
            <span className="w-5 text-right text-[0.5rem] font-semibold text-text">{z.score}</span>
          </div>
        ))}
      </div>
      <div className="mt-3 w-full rounded-lg bg-brand-soft p-2 text-center">
        <p className="text-[0.5rem] font-medium text-brand">✓ Auditoría completada</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="p-3">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-[0.5625rem] font-semibold text-text">Dashboard — Guatemala</span>
      </div>
      {/* Mini map */}
      <div className="relative mb-3 aspect-[2/1] rounded-lg bg-bg-muted">
        {[
          { x: 30, y: 25, c: "#22c55e" }, { x: 45, y: 35, c: "#22c55e" },
          { x: 55, y: 28, c: "#f59e0b" }, { x: 35, y: 50, c: "#22c55e" },
          { x: 65, y: 45, c: "#f59e0b" }, { x: 25, y: 40, c: "#22c55e" },
          { x: 50, y: 55, c: "#ef4444" }, { x: 40, y: 30, c: "#22c55e" },
        ].map((d, i) => (
          <div key={i} className="absolute h-1.5 w-1.5 rounded-full" style={{ left: `${d.x}%`, top: `${d.y}%`, background: d.c }} />
        ))}
      </div>
      {/* Mini stats */}
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={s.label} className="rounded-lg bg-bg-muted p-2 text-center">
            <p className="font-display text-[0.625rem] font-bold text-text">{s.value}</p>
      ...
      <div className="mt-2 rounded-lg bg-bg-muted p-2">
        <svg viewBox="0 0 120 24" className="w-full">
          <polyline points="0,18 20,16 40,12 60,14 80,10 100,7 120,4"
...
const steps = [
  {
    ...
    description: "Define zonas, criterios y puntajes para cada tipo de tienda. Se adapta a tu proceso — no al revés.",
  ...
  },
  {
    ...
    description: "5 minutos por tienda. Checklists con fotos, funciona sin internet. Sin capacitación.",
  ...
  },
  {
    ...
    description: "El vendedor ve el puntaje antes de salir. Sabe exactamente qué mejorar ahí mismo.",
    device: "phone" as const,
    mock: <ScoreMock />,
  },
  {
    ...
    description: "Mapas, tendencias, rankings por territorio. Detecta problemas antes de que escalen.",
    device: "laptop" as const,
